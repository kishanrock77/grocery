...
export class Searchitem implements OnInit {
  ...
  categoryname = '';
  ngOnInit() {
    (document as any).addEventListener('VOICE_TEXT', (event: any) => {
 ...
 opencustomorder() {
  ...
  getCategories() {
    ...
    if (
      ...
      this.api.getLevel2CategoriesOnly(
  ...
  async toggleVoiceSearchg() {
    ...
    try {
      this.common.alertmessage('VOICE search insoide try:', 'info', 'info');
