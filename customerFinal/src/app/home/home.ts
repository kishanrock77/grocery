...
export class Home implements OnInit, AfterViewInit, OnDestroy {
  ...
  itemSubscriptionsMap: Record<string, Subscription> = {};
  ...
  bannerurl(url: any) {
    ...
    this.router.navigateByUrl(appUrl.pathname + appUrl.search);
  }
  getList() {
  ...
  initialteslider() {
    this.timer = setInterval(() => {
      ...
      if (this.current >= this.listbanner.length) {
        this.current = 0;
      }
  ...
  ngOnDestroy(): void {
    ...
    Object.values(this.itemSubscriptionsMap).forEach(sub => sub.unsubscribe());
    this.itemSubscriptionsMap = {};
  ...
  loadCategories(): void {
    ...
    this.categoriesSubscription = this.api.loadCategories(adminId).subscribe({
  ...
  loadStores(): void {
    ...
    this.storesSubscription = this.api.getHomeStores({
  ...
  loadAllItems(): void {
    ...
    this.level1.forEach((cat: any) => {
      ...
      this.itemSubscriptionsMap[cat._id]?.unsubscribe();

      this.itemSubscriptionsMap[cat._id] = this.api.getLevel1Items({
