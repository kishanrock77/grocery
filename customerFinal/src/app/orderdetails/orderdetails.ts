...
export class Orderdetails implements OnInit, OnDestroy {
  ...
  cancelSubOrderSubscription?: Subscription;
  changePaymentMethodSubscription?: Subscription;
  uiDelayTimeout?: ReturnType<typeof setTimeout>;
  ...
  cancelSubOrder(store: any) {
    this.common.confirmopen('Are you sure you want to cancel this order from ' + store.storeInfo?.storeName + '?').then((res) => {
      if (res) {
        this.cancelSubOrderSubscription?.unsubscribe();
        this.cancelSubOrderSubscription = this.api.cancelSubOrder({ customerId: this.orderData.customerId, _id: store._id }).subscribe((response: any) => {
          if (response?.success) {
    ...
    try {
      ...
      this.orderDetailsSubscription = this.api.getorderbyid(
        ...
        if (response?.success) {
          ...
          this.uiDelayTimeout && clearTimeout(this.uiDelayTimeout);
          this.uiDelayTimeout = setTimeout(() => {
            this.uiDelayTimeout = undefined;
  ...
  ngOnDestroy(): void {
    ...
    this.orderDetailsSubscription?.unsubscribe();
    this.cancelSubOrderSubscription?.unsubscribe();
    this.changePaymentMethodSubscription?.unsubscribe();

    if (this.uiDelayTimeout) {
      clearTimeout(this.uiDelayTimeout);
      this.uiDelayTimeout = undefined;
    }
  }
  ...
  changepaymentmethodtocod(orderData: any) {
    this.loading = true;
    this.changePaymentMethodSubscription?.unsubscribe();
    this.changePaymentMethodSubscription = this.api.changepaymentmethodtocod(orderData._id).subscribe({
