...
export class Onlinepayment implements OnInit {
  ...
  ceatebackendorderforazorpay() {
    ...
    this.subs = this.api.ceatebackendorderforazorpay(this.orderId, this.orderData.customerId, this.orderData.totalamount).subscribe((res: any) => {
      this.loading = false;
...
"handler": (res: any) => {
  ...
  completeorder(obj: any, order_id: any, customerId: any) {
    this.loading = true;
    this.api.completeorderforrazorpay(obj, order_id, customerId, this.amount).subscribe((response: any) => {
      this.loading = false;
