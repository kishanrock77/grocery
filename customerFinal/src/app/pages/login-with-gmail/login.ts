...
export class LoginComponent implements OnDestroy {
  ...
  isalreadyloggedin: any=undefined;
  constructor(
    ...
    this.isalreadyloggedin = urlParams.get('isalreadyloggedin');
...
loginwithid(_id: any) {
    ...
    this.loginSubscription = this.auth.loginwithid({
      _id: _id
    }).subscribe((res: any) => {
      this.loading = false;
      if (res.success) {
  ...
  initializeGoogle() {
    ...
    google.accounts.id.initialize({
      client_id: '53907603345-77b74cahufec62hap6odhsfiv6oa4rir.apps.googleusercontent.com',
  ...
  initializeGoogleb() {
    google.accounts.id.initialize({
      client_id: '53907603345-77b74cahufec62hap6odhsfiv6oa4rir.apps.googleusercontent.com',
      use_fedcm: true, // 👈 Change this to true
      callback: (response: any) => {
        this.googleCallback(response);
      }
    });
  }
  googlePromptOpen = false;

  loginWithGoogle() {

    if (this.googlePromptOpen) {
      return;
    }
  ...
  private setShowMobilePopup(value: boolean) {
    this.ngZone.run(() => {
      this.showMobilePopup = value;
    });
  }
