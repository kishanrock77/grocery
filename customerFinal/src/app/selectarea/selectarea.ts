...
export class Selectarea implements OnInit, OnDestroy {
  constructor(
  ...
  selectAreaSubscription?: Subscription;
  ...
  getAreas() {
    this.loading = true;
    this.getAreasSubscription?.unsubscribe();
    this.getAreasSubscription = this.api.getAreas().subscribe((res: any) => {
      this.loading = false;
      if (res.success) {
  ...
  continue() {
    ...
    this.selectAreaSubscription?.unsubscribe();
    this.selectAreaSubscription = this.api.selectareasubmit(
