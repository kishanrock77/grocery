...
export class Orderlist implements OnInit, OnDestroy {
  ...
  getOrders() {
    ...
    try {
      ...
      this.getOrdersSubscription = this.http.get(
  ...
  openMainOrder(order: any) {
    ...
    this.popupService.open('orderdetails', order?.mainOrder?._id + "_main_-1_customer","app");
  ...
  async cancelOrder(
    ...
    try {
      ...
      this.cancelOrderSubscription = this.http.post(
  ...
  changepaymentmethodtocod(order: any) {
    this.loading = true;
    this.changePaymentMethodSubscription?.unsubscribe();
    this.changePaymentMethodSubscription = this.api.changepaymentmethodtocod(order.mainOrder._id).subscribe({
